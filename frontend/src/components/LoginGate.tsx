...
export function LoginGate({
  ...
  fallback,
  blur = false,
  message = 'Sign in to unlock full insights'
}: LoginGateProps) {
  ...
  if (status === 'loading') {
    return (
      <div className="animate-pulse">
        {fallback || <div className="h-24 bg-gray-100 rounded-xl" />}
      </div>
    )
  }
  ...
  // User is not logged in - show gated content
  if (blur) {
    return (
  ...
  // Non-blur mode - show fallback with CTA
  return (
    <div className="bg-gradient-to-br from-gray-50 to-gray-100 rounded-xl border border-gray-200 p-6">
      <div className="text-center">
        {fallback && <div className="mb-4">{fallback}</div>}
        <div className="w-10 h-10 mx-auto mb-3 rounded-full bg-[#10B981]/10 flex items-center justify-center">
